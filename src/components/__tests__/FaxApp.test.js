...
describe('FaxApp', () => {
    ...
    it('-8- should render an error message if user type invalid phone number', async () => {
        ...
        fireEvent.change(input, {target: {files: [file]}})
        const phoneInput = await screen.findByPlaceholderText(`Enter phone number`)
        fireEvent.change(phoneInput, {target: {value: 1234567890123223}})
        const errorMessage = await screen.findByText(`Please enter a valid phone number.`)
        expect(errorMessage).toBeInTheDocument()
    })
    it('-9- should render an error message if user erase phone number', async () => {
        ...
        const errorMessage = await screen.findByText(`Please enter a phone number.`)
        expect(errorMessage).toBeInTheDocument()
    })
})
